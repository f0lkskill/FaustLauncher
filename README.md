...
- **蓝奏云直链本地解析**：所有蓝奏云下载（汉化包 / 字体包 / Mod / 插件 / 图标）在下载前由启动器本地解析成直链，含阿里云 WAF 挑战求解与 302 跳转跟踪，不依赖第三方解析服务；请求自动节流、命中接口限流时进入冷却退避，解析失败回退原链接
- **图标缓存自动维护**：图标下载会校验内容并本地缓存，成功过就不再请求；页面反复重绘不会重复下载，云端已下架的图标缓存会自动清理
- 支持手动更新汉化（首页「🎯 汉化更新」按钮）
